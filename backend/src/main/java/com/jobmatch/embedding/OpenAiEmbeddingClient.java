...
/**
 * Production embedding adapter — calls the OpenAI embeddings API. Active when
 * {@code embedding.provider=openai}; requires {@code OPENAI_API_KEY} (fails fast if missing).
 */
@Component
    ...
    public OpenAiEmbeddingClient(
        ...
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException(
                    "OPENAI_API_KEY must be set when embedding.provider=openai");
        }
        this.model = model;
