...
@Testcontainers
class AtsTailorIntegrationTest {
    ...
    static void properties(DynamicPropertyRegistry registry) {
        ...
        // chat.provider defaults to "fake" -> canned ATS JSON + canned stream.
    }
    ...
    @Test
    void ats_returnsStructuredScore() {
        String token = registerAndGetToken("ats-user@example.com");
        String resumeId = uploadResume(token);
        String jobId = createJob(token);

        ResponseEntity<JsonNode> ats = rest.exchange(
                "/api/resumes/" + resumeId + "/jobs/" + jobId + "/ats",
                HttpMethod.GET, new HttpEntity<>(bearer(token)), JsonNode.class);

        assertThat(ats.getStatusCode()).isEqualTo(HttpStatus.OK);
        JsonNode body = ats.getBody();
        assertThat(body.get("score").asInt()).isBetween(0, 100);
        assertThat(body.get("matchedKeywords").isArray()).isTrue();
        assertThat(body.get("missingKeywords").isArray()).isTrue();
        assertThat(body.get("summary").asText()).isNotBlank();
    }

    @Test
    void tailor_streamsAsSse() {
        String token = registerAndGetToken("tailor-user@example.com");
        String resumeId = uploadResume(token);
        String jobId = createJob(token);

        ResponseEntity<String> tailor = rest.exchange(
                "/api/resumes/" + resumeId + "/jobs/" + jobId + "/tailor",
                HttpMethod.GET, new HttpEntity<>(bearer(token)), String.class);

        assertThat(tailor.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(tailor.getHeaders().getContentType().toString()).contains(MediaType.TEXT_EVENT_STREAM_VALUE);
        assertThat(tailor.getBody()).contains("data:");
    }
    ...
    private String registerAndGetToken(String email) {
        ResponseEntity<JsonNode> reg = rest.postForEntity(
                "/api/auth/register",
                Map.of("email", email, "password", "supersecret", "fullName", "ATS User"),
                JsonNode.class);
